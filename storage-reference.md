...
The current contract is rc.17: [concise console output](#concise-console-output--c) includes startup information and the tagger's disk-load counts, retaining sorter `PASS`, `TAKE`, or `FAIL` without a trailing destination clause. These display changes add no files or storage conversion. Tagged children retain the rc.14 [child naming contract](#message-state-suffixes) and [upgrade guidance](README.md#child-naming-upgrade). Sender IDs may be any usable literal directory component, without a UUID requirement. See the [sender-ID upgrade note](README.md#sender-id-upgrade) before introducing IDs that older binaries cannot load. The rc.11 explicit tagger trace path and rc.10 mail layout remain unchanged: mail working state is under the selected spool's Proc subtree, while sender configuration and permanent mappings retain the rc.9 data layout. There is no on-disk schema/version marker, automatic migration, or fallback to earlier locations or meanings. Installations predating rc.10 must follow the [offline queue migration](queue-layout-2026-09-17.md), including earlier configuration conversions when applicable. Use [bring-up.md](bring-up.md) for staged setup and [deployment.md](deployment.md) for installing and running the executables. Dated experiment reports describe versions tested at that time; they are evidence, not alternate current storage definitions.
...
`<spooldir>` is the configured SmarterMail spool root, not its `proc` child. `<mailroot>` is a documentation shorthand for the fixed `<spooldir>\proc\sm-tagger` directory, not another CLI argument. `<datadir>` is the separate configuration/mapping root. Relative command-line roots and both programs' log paths resolve against the process working directory; prefer absolute paths. These trees describe possible contents, not a list to create in full before first use.

All mail transitions stay on the spool's ordinary local NTFS volume: Proc, the fixed mailroot, its `process`/`failed` children, and final spool output. Do not redirect these paths through junctions or other reparse points. The datadir may be on a different ordinary local NTFS volume; its mapping `staging` and `tag-addresses` directories must share that data volume for atomic mapping publication. Auth-index staging likewise stays with its destination root. Placement, permissions, and topology are trusted installation conditions, without runtime ACL, volume, or reparse-point auditing. Keep binaries and generated build output separate from mail/configuration paths. Use dedicated optional log files outside mail queues and configuration/mapping records; a separate nonrecord log file in the datadir root is permitted.
...
- **`senders\sender-ids\<sender-id>\`** — one complete tagger sender-id record per stable sending identity (§1.2). Choose its folder name once under the [sender-id rules](#identifiers-and-addresses), then never rename or reuse it.
...
Write auth-address index and tag-address folder names in canonical lowercase; preserve sender-id spelling exactly. All obey the [literal Windows name rules](#literal-windows-names); an otherwise valid auth-address that cannot be a literal Windows component is not enrollable in version 1.
...
| `<datadir>` | Administrator provisions before bring-up. Tagger startup needs it for the data lock before configuration loading. |
...
| Parent of either program's `-l <logfile>` | Administrator creates. A missing/inaccessible parent only disables this best-effort log. |
...
| `tag-addresses\<tag>\tag-log.txt` | Tagger creates/appends best effort, independently of `-l` and `-v` | Administrator; neither program parses old contents | Not required for mapping validity, not delivery proof. |
| Chosen sorter email log | Sorter with `-l` | Administrator | Append-only; separate from tagger trace. |
| Chosen tagger execution trace | Tagger with `-l` | Administrator | Append-only; no implicit datadir trace path. |
...
- **Sender-id:** any nonempty usable [literal Windows directory component](#literal-windows-names), chosen once for a sender and never renamed or reused. No UUID, email-address, lowercase, or other identity-specific shape is required. For example, `Sales Team`, `Alice`, and `Équipe` are valid; a UUID remains valid too. The directory name, auth-index pointers, and mapping pointers must have exactly the same spelling, including case, spaces, and Unicode representation. No case folding, whitespace trimming (apart from permitted terminal CR/LF in files), or Unicode normalization is applied. It is a local reference key in configuration, mappings, and diagnostics; the programs do not insert it into email.
...
- **Private-address:** the configured tagging trigger. Its value may also be an auth-index key for the same sender-id. This role has no separate directory.
...
Persistent identity files use strict UTF-8 with no added encoding preamble. Their reader decodes every byte and removes only permitted terminal CR/LF; it does not trim spaces or strip an optional BOM. Spaces that are valid within a sender-id are part of that exact ID, including a leading space. The three editable sender settings have a separate convenience rule: remove leading U+FEFF characters (including an editor's UTF-8 BOM), then allow surrounding Unicode whitespace. Private addresses and MDN policy still require valid UTF-8 and their complete value grammar; template annotations have the exception below. This setting normalization never applies to sender-ID pointers or permanent mapping identities. Auth and tag directory names must already be lowercase canonical addresses.

**`sender-id.txt`** — In an auth-index directory, points that canonical auth-address to its stable sender-id record. In a tag-address directory, identifies the sender-id record that owns the permanent mapping. In both roles, remove only permitted terminal CR/LF, require one usable sender-id component and no other content, and require an exact ordinal match to an existing `senders\sender-ids\<sender-id>\` directory name. Windows may resolve some paths without regard to case; that does not make differently spelled ID pointers equal. Write and close an auth-index pointer before publishing its directory. Changes to existing indexes require the stopped-tagger configuration-update procedure below; permanent tag-mapping identity files remain immutable.

No optional BOM is recognized in `sender-id.txt`. Bytes `EF BB BF` decode to literal U+FEFF as part of the ID, so an editor-added BOM normally makes the pointer fail to match the intended directory. If the chosen directory name itself begins with U+FEFF, those bytes represent the actual ID and must remain present in every pointer; they are not removed or added separately as an encoding marker.

**`private-address.txt`** — In a sender-id record, holds exactly one current private-address. Remove leading BOM characters and trim surrounding whitespace, then parse and canonicalize the complete address. ASCII uppercase is accepted. Embedded whitespace, comments, and extra addresses remain invalid.

Published auth-directory names and configured private addresses are validated under [address identity](#address-identity) at startup. Each canonical address belongs to one sender-id. An auth address may equal that same sender-id's private address, including case-only differences in the configured private value. Private/private or auth/private overlaps across different sender IDs remain invalid. Several different auth addresses pointing to the same sender-id are permitted. Authentication selects the record; only supported sender fields activate tagging, and HDR `auth:` remains unchanged even when its value equals the private address. Missing, unreadable, malformed, duplicate, or conflicting live values are startup configuration failures. Former private addresses are neither loaded nor blocked; after replacement they are ordinary non-matching identities.

**`allow-mdn.txt`** — Required per sender-id record. After removing leading BOM characters and trimming surrounding whitespace, its complete contents must be `false` or `true`, compared without case sensitivity. `False`, `TRUE`, and padded values are accepted; comments, embedded whitespace, multiple values, numeric values, and `yes`/`no` are invalid. `false` is the initial and recommended value. `true` permits recognized outbound MDN responses for this sender-id and may be deployed only after the evidence/approval gate in §1.4 covers every account and path sharing it. Adding an auth index to an enabled record requires extending that evidence before use. This is configuration, not a claim that the program rewrites MDN bodies.

### `from-template.txt`

Remove leading BOM characters before applying the existing whitespace/first-token rule. An **address pattern only** — no display name, no angle brackets. A single `%` marks where the token goes:
...
A live `tag-addresses\<tag-address>` directory has two required identity files: `sender-id.txt` (the same exact sender-id format as an auth-index pointer) and `recipient-id.txt`. The tagger writes both as UTF-8 without an added encoding preamble or terminal newline, closes them, and publishes their complete containing directory from `staging\<uuid>.tagtmp` without replacement. A separate `tag-log.txt` is optional diagnostic data.
...
Pass-through output keeps the SmarterMail-assigned input basename. Each tagged child uses `<input-basename>c<n>`: preserve the entire input basename exactly, append one literal lowercase `c`, then its one-based canonical child ordinal (§7.4) in invariant decimal ASCII digits without leading zeroes. For example, input `42615432` produces `42615432c1`, `42615432c2`, through `42615432c10` and beyond; `-42615432` produces `-42615432c1`. Do not parse, normalize, or strip any part of the parent name. The same child basename identifies its `.process`, `.pend`, `.out`, live pair, trace, diagnostic, and tag-log entry. The common parent portion supports searching all children in SmarterMail's Delivery log.

**VERSION-SENSITIVE-003, VERSION-SENSITIVE-015:** The `c` delimiter remains compatible with the observed hexadecimal-style Delivery session parser; the earlier hyphen delimiter can break Related Traffic grouping on the locally tested SmarterMail build. Recheck exact child and whole-parent searches after upgrades. The server may shorten its displayed session IDs; those IDs are not a global uniqueness guarantee. The namespace/retention assumption remains separate from log compatibility. Earlier hyphenated output and retained states keep their actual names and receive no automatic conversion, replay, or recovery; see the [child-naming upgrade note](README.md#child-naming-upgrade).
...
**Sorter console debugging (`-v`)** — best-effort standard-output lines prefixed `DEBUG`. Lifecycle, queue scans, readiness deferrals, routing decisions, file-operation intents/results, and stale entries identify their `event`; a terminal message summary uses the email record's result fields. A readiness deferral uses `event=DEFER` with reason and input paths. This output is independent of `-l` and `-c` and includes no EML contents or tagger-only configuration/mapping inspection. Without `-v`, no debug output is attempted. A console formatting/write failure disables debug output for the invocation and attempts a standard-error report, without affecting mail or the email log. Ordinary errors still go to standard error regardless of output options.

**Tagger execution trace (`-l <logfile>`)** — the optional append-only detailed operational trace at the selected path. It retains the trace event format; it is not a one-line-per-message sorter log. No execution-trace file is accessed without `-l`, and an old `<datadir>\log.txt` remains untouched unless explicitly selected. The administrator provides the parent directory and a dedicated file outside queues, configuration files, mapping records, and the sorter log. Relative paths resolve from the working directory; prefer absolute paths. Encode each new event as UTF-8 without a BOM and terminate it with CRLF. Every new event begins with the fixed timestamp/run/seq/context/event/result prefix below and may continue with any number of human-readable `name=value` details:

```
2026-09-01T14:30:12.123Z run=0d3b3c3d-6ca6-4c11-84a0-b119576fa3c8 seq=42 context="message-42" event=STATE_RESULT result=ERROR operation="move" source="D:\\SmarterMail\\Spool\\proc\\sm-tagger\\process\\message-42c1.eml.pend" destination="D:\\SmarterMail\\Spool\\message-42c1.eml" error="destination exists" residual="EML=.eml.pend; HDR=.hdr.pend; later children=.pend; parent=.break"
```
...
In file records, quote every arbitrary string value. Within it, escape `\` as `\\`, `"` as `\"`, CR as `\r`, LF as `\n`, HTAB as `\t`, and any other control or byte not representable as valid UTF-8 as `\xHH`. Integers and fixed uppercase tokens may be unquoted. Format exception values once and escape them once, without an extra nested quote layer. These rules prevent newly encoded values from introducing additional physical lines while retaining the complete relevant value; they do not establish the integrity of existing log contents. Human-readable console exception rendering is defined separately below.

The sorter email log and console debugging, both programs' concise console summaries, tagger file/console execution traces, parent diagnostics, retained `-keep` artifacts, and standard-error diagnostics are protected operational data. The programs perform **no redaction**. Any configured private-address, auth-address, sender-id, recipient-id, tag-address, basename, full path, parsed metadata/header value, error text, or other datum relevant to the event may and should appear. “Relevant” means read, derived, compared, selected, rejected, mutated, or returned by the program; it does not require dumping an EML body or other bytes the program never examines. These outputs are not sanitized or safe for release outside the trusted operator/security zone.

With `-l` or `-v`, record enough information for a human to reconstruct the path taken: startup/options, configuration loading, queue scans, message ownership, auth/sender-id record selection, important parsed and canonical values, mapping lookup/allocation, branch decisions, child order, publication, shutdown, and failures.
...
After both tagger locks are held and before configuration is loaded or messages are examined, `-l` attempts to open the selected file as the invocation's append-only trace; `-v` independently enables console tracing. Neither output is initialized before both locks. Only that tagger writes the file; other processes may read it but must not write to it while the tagger runs. Do not automatically rotate, rename, truncate, or delete it. An operator may archive or clear it only while `sm-tagger` is stopped and accepts that a long logging session can grow without an internal bound. The file-lifetime and append mechanics belong in `implementation.md`.

If opening or using the requested trace file fails, attempt a standard-error diagnostic beginning `ERROR logging to "<full-log-path>":` with the failed event and known residual state when available. Disable only file tracing for the rest of that invocation and continue normal startup, requested console tracing, current-message processing, later messages, and shutdown. Do not repeatedly reopen or retry the failed trace; a later invocation may try again. A close failure is likewise reported without changing the mail outcome or exit status. Standard error is best effort and is not recursively logged; inability to write it creates no additional fallback. The managed operations used by logging are defined in `implementation.md`.

Independently of either program's `-l`, `-v`, or `-c` flags, attempt a complete human-readable standard-error diagnostic for startup/configuration failures, fatal watcher failures, runtime message-contract errors, and all logging or diagnostic-file failures. Print exception chains and stack traces with their actual line breaks and literal path backslashes. Do not decode escape sequences from previously formatted strings: a literal backslash followed by `n` in a path must remain literal. Scalar message metadata retains control escaping. Exception text is a multiline operator diagnostic, not a single-line record or a sanitized presentation. Standard error is best effort; inability to write it creates no fallback. Out-of-band collection is deferred to FUT-005 in `future.md`.

**Tagger console debugging (`-v`)** — write the same execution events to standard output, prefixed `DEBUG `, and flush after each event. When both outputs are enabled, their values, run ID, and sequence agree, but console rendering leaves path backslashes literal and prints exception objects unquoted with actual line breaks. Other scalar controls stay escaped. Each value is converted once before its file and console representations are produced. Neither `-v` nor its absence changes per-mapping tag-log attempts. A console write/flush failure disables only verbose output for the invocation and attempts stderr reporting; the trace file and mail processing continue. Never close the caller-owned console writer. Shared event-formatting or run-ID initialization failure disables all requested execution-trace outputs best effort, reports stderr, and leaves mail processing unaffected. Verbose output remains quiet when `-v` is absent; `-c` independently enables concise summaries.

### Concise console output (`-c`)

Both programs independently accept `-c` to write and flush startup information and one standard-output line after each complete output-pair transfer. All lines begin with server-local time `yyyy-MM-dd HH:mm:ss`, a colon and space. After acquiring all required locks, each program attempts these two startup lines:

```text
2026-09-19 15:36:00: sm-tagger 1.0.0-rc.17 starting, watch mode
2026-09-19 15:36:00: data "D:\sm-tagger-data", spool "D:\SmarterMail\Spool"
```

The first line uses the running application's name and version. For one-shot mode, its ending is `starting, one-shot mode, message <basename>`. The second line reports the resolved absolute directories. The tagger prints these lines before loading configuration; `starting` does not claim a successful load, an installed watcher, or readiness to process mail. Invalid arguments or lock acquisition failure produce no concise startup lines. A later startup failure may leave these welcome lines followed by the normal stderr diagnostic.

After configuration and the tag store have both loaded successfully, and before queue processing, the tagger also attempts:

```text
2026-09-19 15:36:00: loaded 2 sender records and 37 tag mappings from disk
```

Count the successfully loaded sender-id records, not auth indexes, and all live individual and group tag mappings. Inert staging is excluded. Use the already loaded collections without extra disk reads or scans. The line is a startup snapshot, not a running count, and is absent if either load fails. The sorter never loads or counts sender records or mappings.

For mail summaries, the format is server-local time `yyyy-MM-dd HH:mm:ss`, a colon and space, then basename, sorter outcome or tagger `from`, original positional HDR envelope sender, envelope recipients, and authentication status. Tagger lines also include an optional individual-tag result and destination:

```text
2026-09-18 15:36:00: 42615432 PASS <sender@example.com> to <recipient@example.net>, no auth
2026-09-18 15:36:01: 42615433 TAKE <private@example.com> to <alice@example.net>, <bob@example.org>, auth <account@example.com>
2026-09-18 15:36:02: 42615433c1 from <private@example.com> to <alice@example.net>, auth <account@example.com>, created tag <tag-83f93@reply.example.com>, moved to spool
2026-09-18 15:36:02: 42615433c2 from <private@example.com> to <bob@example.org>, auth <account@example.com>, used tag <tag-c17b4@reply.example.com>, moved to spool
2026-09-18 15:36:03: 42615434 FAIL <> to unknown, auth unknown
```

The sender comes from original HDR positional line 2, before any tag rewrite; `to` comes from positional line 3, or the individual child's envelope recipient after fan-out. Neither visible EML fields nor HDR `from:` metadata substitute for those positional values. Enclose known displayed addresses in angle brackets and separate multiple recipients with comma-space. An empty sender or the null reverse-path is `<>`; unavailable sender or recipient information is `unknown`. Authentication displays `auth <canonical-address>` when established, `no auth` only when absence is established, and `auth unknown` otherwise. Display extraction is best effort: it does not validate positional addresses or change mail acceptance, and the sorter does not open the EML or load tagger-only data for this output.

The sorter reports the unchanged original basename followed by `PASS` for a pair moved to `spool`, `TAKE` for a pair moved to `process`, or `FAIL` for a pair retained in `failed`. The outcome replaces `from`, and sorter lines have no trailing `moved to` clause. `proc` remains the sorter input. These concise labels do not change the `PASS`, `DIVERT`, or `ERROR` records selected by `-l`. The tagger reports the output child basename and `created tag <address>` or `used tag <address>` for its individual tag, then `moved to spool`. `created` means this message's mapping lookup allocated that individual mapping; `used` means the lookup found an existing mapping. It does not establish final delivery; publication-attempt logs may already have been attempted for a newly created mapping. Group-tag details are omitted from concise output. Tagger pass-through reports the original basename, original envelope values, and `moved to spool`, without a tag clause.

A mail-summary line is attempted only after both EML and HDR moves succeed, including complete sorter retention of `Failed` mail with its still-suffixed HDR. Complete retention of upstream `Failed` mail prints `FAIL` while keeping its existing stderr and error outcome. There are no concise mail summaries for deferrals, stale entries, partial or failed moves, internal ownership/retention suffix changes, mapping creation alone, pending children, or parent cleanup. Each published child gets its own line immediately; later sibling or cleanup failure does not undo that completed transfer. No cross-message reporting state or deduplication is required. A successful pair transfer does not establish final delivery.

Escape control characters, including CR, LF, HTAB, and Unicode line separators, so each startup or mail-summary line occupies one physical line. These are unredacted operational summaries inside the same protected boundary as other diagnostics. The same output sink handles startup and mail summaries. Formatting, writing, or flushing failure attempts stderr reporting and disables only concise output for the invocation; mail processing, verbose output, selected file logs, and exit status continue unaffected. Do not close the caller-owned console writer or retry a failed concise output during that invocation.

`-c`, `-v`, and `-l` are independent. Both console flags produce both outputs when supplied together. `-c` does not change `-l` file records, tag-log attempts, trace event sequences, or their UTC timestamps; its timestamps alone use server-local time. Without `-c`, no concise startup information or mail summary is attempted. Without either console flag, stdout remains quiet.
