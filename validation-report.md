...
- [Validation command output](artifacts/test-results/validation-v1.0.0-rc.3.txt).
- [Release file and package SHA-256 manifest](artifacts/release/manifest.json).
- [Independent artifact verification](artifacts/test-results/artifact-verification-v1.0.0-rc.3.json).
...
The current release folders, standalone EXEs, ZIPs, and manifest represent **v1.0.0-rc.3**. Public assets and their manifest are available from [the GitHub release](https://github.com/bigjosh/sm-tagger2/releases/tag/v1.0.0-rc.3). Raw local evidence and private messages are not published.
...
| Publication | `1.0.0-rc.3`, Release, self-contained `win-x64`; application folders and single-file EXEs; no trimming |
...
| Sorter, invocation, and queue lifecycle | 52 | HDR-only routing, unread EML pass-through, ownership ordering, literal basenames, independent singleton locks, sorted scans, native notification arrivals, overflow/error paths, stopping after owned work |
| Published executable processes | 8 | End-to-end sorter/tagger operation, exit status and logging failures, independent watchers, live sorter log flushing for startup and newly arriving mail, singleton contention, startup-failure isolation, crash/restart, release dependency isolation |
...
| **Total** | **375** | **All executed; none skipped** |
...
- [Standalone sorter EXE](https://github.com/bigjosh/sm-tagger2/releases/download/v1.0.0-rc.3/sm-sorter.exe)
- [Standalone tagger EXE](https://github.com/bigjosh/sm-tagger2/releases/download/v1.0.0-rc.3/sm-tagger.exe)
...
The [release manifest](https://github.com/bigjosh/sm-tagger2/releases/download/v1.0.0-rc.3/manifest.json) is the authoritative SHA-256 inventory for the EXEs, runtime notices, complete application folders, and ZIPs.
...
Every external `VERSION-SENSITIVE-*` gate in [assumptions.md](assumptions.md) remains **PENDING**. All fourteen identifiers are also present beside dependent source behavior. No production mail queue, DNS configuration, account, or server was changed during implementation.
...
There are no known unresolved implementation blockers from this local review. Production enablement still depends on the recorded deployment gates.
