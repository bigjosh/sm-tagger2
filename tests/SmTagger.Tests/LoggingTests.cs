...
public sealed class LoggingTests
{
    ...
    public void DisabledTraceDoesNotOpenOrFormatAnything()
    {
        ...
        int opens = 0;
        using var trace = TraceLog.OpenForTesting(fixture.Root, false, fixture.Errors, () => FixedTime,
    ...
    public void TraceUuidFailureReportsAndContinuesWithoutOpeningLog()
    {
        using var fixture = new StoreTestFixture();
        int opens = 0;
        using var trace = TraceLog.OpenForTesting(fixture.Root, true, fixture.Errors, () => FixedTime,
            (_, _) => { opens++; return new MemoryStream(); },
    ...
    public void TraceHasStablePrefixAndPreventsLineInjection()
    {
        ...
        using (var trace = TraceLog.OpenForTesting(fixture.Root, true, fixture.Errors, () => FixedTime, (_, _) => bytes))
        {
    ...
    public void CorruptTraceIsAppendedWithoutReadingOrRepair()
    {
        ...
        using (var trace = TraceLog.Open(fixture.Root, true, fixture.Errors, () => FixedTime))
    ...
    public void TraceFailureNeverGatesMutationAndNeverReopens(string stage)
    {
        using var fixture = new StoreTestFixture();
        int opens = 0;
        using var trace = TraceLog.OpenForTesting(fixture.Root, true, fixture.Errors, () => FixedTime, (_, _) =>
        {
            opens++;
            if (stage == "open")
    ...
    {
        using var fixture = new StoreTestFixture();
        using var trace = TraceLog.OpenForTesting(fixture.Root, true, fixture.Errors, () => FixedTime, (_, _) => new MemoryStream());
    ...
    {
        using var fixture = new StoreTestFixture();
        TraceLog trace = TraceLog.OpenForTesting(fixture.Root, true, fixture.Errors, () => FixedTime,
    ...
    {
        using var fixture = new StoreTestFixture();
        using var trace = TraceLog.OpenForTesting(fixture.Root, true, new ThrowingTextWriter(), () => FixedTime,
    ...
    public void PublicationLogsAreDistinctOrdinalAndShareOneTimestamp()
    {
        ...
        using var trace = TraceLog.OpenForTesting(fixture.Root, false, fixture.Errors, () => FixedTime, (path, _) =>
        {
            opens.Add(path);
        ...
        store.AppendPublicationLogs([maps[0], maps[1], maps[0]], "parent-2", "parent");
        ...
        Assert.All(buffers, buffer => Assert.Equal("20260905T103456Z parent-2\r\n", Encoding.UTF8.GetString(buffer.ToArray())));
    }
    ...
    public void FirstTagLogFailureContinuesRemainingTagsAndLaterChildren()
    {
        ...
        store.AppendPublicationLogs(maps, "parent-1", "parent");
        store.AppendPublicationLogs(maps, "parent-2", "parent");
    ...
    {
        using var fixture = new StoreTestFixture();
        using var trace = TraceLog.OpenForTesting(fixture.Root, false, fixture.Errors, () => FixedTime, (path, mode) =>
        {
        ...
        store.AppendPublicationLogs([mapping], "parent-1", "parent");
        Assert.Equal("20260905T103456Z parent-1\r\n", File.ReadAllText(Path.Combine(mapping.DirectoryPath, "tag-log.txt")));
    ...
    public void TagLogOperationFailureIsNonblockingAndLaterEntryRetriesNormally(string stage)
    {
        ...
        int opens = 0;
        using var trace = TraceLog.OpenForTesting(fixture.Root, false, fixture.Errors, () => FixedTime, (_, _) =>
        {
            opens++;
            return new FailingLogStream(stage);
        });
        TagStore store = TagStore.Load(fixture.Root, fixture.LoadConfiguration(), trace, clock: () => FixedTime);
        store.AppendPublicationLogs(store.Mappings.Values, "parent-1", "parent");
        store.AppendPublicationLogs(store.Mappings.Values, "parent-2", "parent");
