...
These commands use rc.17 [concise console output](README.md#concise-console-output) with startup information and retained sorter `PASS`/`TAKE`/`FAIL` labels, rc.14 [child naming](README.md#child-naming-upgrade), rc.13 [configuration/diagnostics behavior](README.md#configuration-and-diagnostics-upgrade), rc.12 sender-ID rules, rc.11 logging options, the rc.10 mail workspace, and rc.9 enrollment semantics. Existing sender IDs remain unchanged; see the [sender-ID upgrade note](README.md#sender-id-upgrade). Update older launchers using the [CLI upgrade note](README.md#tagger-cli-upgrade). Before using an installation predating rc.10, complete the [offline queue migration and rollback procedure](queue-layout-2026-09-17.md), including earlier configuration conversions as needed, with both programs stopped and upstream processing paused. Do not leave mail in an old datadir queue or former retired indexes unreviewed: there is no fallback, and every published index is active. The steps below otherwise assume fresh storage.
...
There are no additional directory requirements: stage 1 already created the email log's parent directory. `-l` appends a terminal `PASS`, `DIVERT`, or `ERROR` line for a processed message to `sorter-mail.log`; `-v` shows debugging on the console's standard output. `-c` optionally prints the program/version, selected mode, and resolved data/spool paths at startup, then one concise line after the complete pair moves, using `PASS` for `spool`, `TAKE` for `process`, or `FAIL` for `failed`; see the [concise format](storage-reference.md#concise-console-output--c). All three flags are independent; using both console flags prints both outputs. Without `-l`, the sorter opens no email log; without `-v`, it omits debug output. Errors still go to stderr. The sorter does not create a missing log parent directory.

A successful one-shot returns `0`; the pair leaves `proc` and is published EML-first/HDR-last into `$spool`, where SmarterMail may consume it immediately. Use controlled queue observation to compare the unchanged pair bytes, and confirm delivery at the independent test receiver. Repeat for authenticated non-enrolled and no-auth paths. Establish readiness and route coverage under `LAB-001` and `LAB-002`. The email-log result and any concise mail summary describe the sorter's operation, not final delivery.
...
& $tagger $data -l (Join-Path $evidence 'tagger-trace.log') -v $spool 2> (Join-Path $evidence 'tagger-empty.stderr.txt')
$LASTEXITCODE
```

Run the tagger in a separate console from the sorter. The selected trace file records detailed execution events; `-v` also shows those events on stdout. Replace `-v` with `-c` for startup information and one line per completely moved output pair, or specify both to see both outputs; the file trace stays unchanged. The protected evidence directory already exists from stage 1; there is no implicit datadir log or automatic log-parent creation. Both outputs are best effort and independent, and errors always go to stderr. Confirm its required roots using [required directories and creation](storage-reference.md#required-directories-and-creation); that reference also identifies generated queues, staging, locks, and logs.

With `-c`, check the resolved paths and the `loaded 0 sender records and 0 tag mappings from disk` summary for this empty setup. The welcome alone does not establish successful loading or watcher readiness. Check startup stderr, both held tagger locks, and the created `$mailroot\process` directory, then request an orderly stop and inspect its exit status. This establishes empty-configuration startup. It does not yet establish rewriting. Do not place non-enrolled messages directly into `process`: the tagger requires a consistent enrolled auth in its startup configuration.
...
Choose a permanent sender-id and prepare its complete record and staged auth index using the authoritative [sender-configuration files and formats](storage-reference.md#sender-configuration). A descriptive name is permitted under the [identifier rules](storage-reference.md#identifiers-and-addresses); the example UUID is optional. [The configuration example](examples/README.md) supplies synthetic contents to adapt; replace its identities and keep the chosen sender-id spelling identical in its directory name and index pointers. Keep MDNs disabled for this initial trial and leave the process queue empty.
...
& $tagger $data -l (Join-Path $evidence 'tagger-trace.log') -v -keep $spool $basename 2> (Join-Path $evidence ($basename + '.tagger.stderr.txt'))
...
| Concise console output | With `-c`, confirm the startup version/mode/paths and loaded sender-record/tag counts, then compare each completed pair transfer with exactly one mail-summary line. Tagged children include their child ID, individual recipient, and created/used individual tag; pass-through keeps its original basename without a tag clause. A later sibling or cleanup failure must not retract already published child lines. Check `-c` alone and alongside `-v`/`-l`; the independent file formats and stderr behavior must stay unchanged. |
| Child names and outbound logs | Outputs follow the [child naming contract](storage-reference.md#message-state-suffixes). Compare receiver evidence and raw Delivery logs; verify that a parent-basename search with Related Traffic includes all and only that parent's children on this server. Follow the [matching-only/raw-log fallback](deployment.md#observe-failures) if the viewer misgroups results (`LAB-015`). |
