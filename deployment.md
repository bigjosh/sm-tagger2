...
For sorter-only pass-through, create an empty, accessible `<datadir>\senders` directory. The existing spool root and its `proc` directory must be ready before startup. `profiles`, `process`, and mapping directories are not needed for that stage; malformed or ambiguous HDRs still fail closed.
...
The programs never retry or repair retained mail or staging entries automatically. Restart loads complete permanent mappings, reports inert leftovers, and processes only fresh plain HDRs. Stop the relevant owners before administrative changes or disposition. Automated notification, inbound processing, and recovery remain deferred in [future.md](future.md).
